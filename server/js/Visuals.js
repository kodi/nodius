...
// Circle
Visuals.prototype.circle = function (x, y, radius, fill, colorHexFill, colorHexStroke, alphaFill,alphaStroke) {

    this.ctx.lineWidth = this.getLineWidth();
    this.ctx.beginPath();

    if (typeof(alphaFill) == 'undefined') {
        this.ctx.fillStyle = colorHexFill || this.defaultColor;
        this.ctx.strokeStyle = colorHexStroke || this.defaultColor;
    } else {

        var colorR = parseInt(colorHexFill.substring(0, 2), 16);
        var colorG = parseInt(colorHexFill.substring(2, 4), 16);
        var colorB = parseInt(colorHexFill.substring(4, 6), 16);
        this.ctx.fillStyle = 'rgba(' + colorR + ',' + colorG + ',' + colorB + ',' + alphaFill + ')';
        ...
        this.ctx.strokeStyle = 'rgba(' + colorRF + ',' + colorGF + ',' + colorBF + ',' + alphaStroke + ')';
    }
    //DRAW CIRCLE
...
Visuals.prototype.text = function (text, x, y, fontsize, colorHexStroke, alpha, angle) {
    ...
    if(this.hasCanvasTextSupport){
    //if(false){  used when debuging non html5 browsers //
        ...
        if (angle == 90) {
            ...
            this.ctx.translate(y-10, (this.containerHeight/2)+24-this.containerHeight);
